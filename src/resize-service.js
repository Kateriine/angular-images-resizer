...
(function(window, angular) {
  ...
  angular
    ...
    .service('resizeService', ['$q', '$document', '$window', function ($q, $document, $window) {
      ...
      this.cropCanvas = function (cnv, width, height, cnvW, cnvH) {
          if (!width || !height) {
              return cnv;
          }
          cnvW = !cnvW ? width : cnvW;
          cnvH = !cnvH ? height : cnvH;
          var   posX = cnvW <width? (cnvW-width)/2 : 0;
          var   posY = cnvH <height? (cnvH-height)/2 : 0;
          var tmpCanvas = $document[0].createElement('canvas');
          
          tmpCanvas.width = cnvW;
          tmpCanvas.height = cnvH;
          var cnx = tmpCanvas.getContext('2d');
          cnx.drawImage(cnv, 0, 0, width, height, posX, posY, width, height);
          return tmpCanvas;
      };
      ...
      this.resizeImage = function (src, options) {
          ...
          options = {
              ...
              crop: options.crop ? options.crop : false,
              outputFormat: ext === 'png' ? 'image/png' : 'image/jpeg',
          ...
          service
              ...
              .then(function (img) {
                  if (options.height || options.width) {
                      deferred.resolve(service.resizeImageWidthHeight(img, options.width, options.height, options.step, options.outputFormat, options.crop));
                  }
       ...
       */
      this.resizeImageWidthHeight = function (image, width, height, step, outputFormat, crop) {
          if (!image) {
              return null;
          }
          if (!outputFormat) {
          ...
          mainCanvas.getContext('2d').drawImage(image, 0, 0, mainCanvas.width, mainCanvas.height, 0, 0, mainCanvas.width, mainCanvas.height);
          var i = 0;
          for (i = 1; i < step; i++) {
          ...
          if(crop)
            mainCanvas = this.cropCanvas(mainCanvas, width, height, destW, destH);
